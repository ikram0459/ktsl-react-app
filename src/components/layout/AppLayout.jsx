...
function WhatsAppButton() {
  return (
    <a href="https://wa.me/353892444612" target="_blank" rel="noopener noreferrer" aria-label="Chat on WhatsApp" className="fixed bottom-6 left-6 z-40 bg-green-500 hover:bg-green-600 text-white p-3 rounded-full shadow-lg transition-transform hover:scale-110" data-ga-event="whatsapp_click" data-ga-category="engagement" data-ga-label="WhatsApp Chat Button" data-testid="button-whatsapp">
...
export default function AppLayout() {
  ...
  useEffect(() => {
    const handleCapture = (event) => {
      ...
      const label = target.dataset.gaLabel || target.textContent?.trim() || window.location.pathname;
    ...
    document.body.addEventListener("click", handleCapture);
    return () => document.body.removeEventListener("click", handleCapture);
  }, []);
